...
define(['modules/api', 'modules/jquery-mozu', 'hips/io'], function(api, $, io) {

  var preload = (function preload(subs) {
    var subscriptions =  [];
    var config = {
      registered: {
        //myaccount: ['rewardPoints', 'segment', 'recentPurchases']
      },
      anonymous: {

      },
      both: {
        //'search': [{pre: ['location'], req: 'productListInformation'}]
      },
      'custom': [
        /*
        function(){
          if(require.mozuData('pagecontext').pageType !== 'category') return null;
          else return [{pre:['location'], req: 'productListInformation'}];
        }
        */
        function() {
          return ['customer'];
