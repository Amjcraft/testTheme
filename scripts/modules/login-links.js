...
define([
    ...
    'modules/api',
    'hyprlive',
    'underscore', 'vendor/jquery-placeholder/jquery.placeholder'
  ],
  ...
  function ($, api, Hypr, _) {
    ...
    $.extend(DismissablePopover.prototype, {
      ...
      onPopoverShow: function () {
        ...
        if (polyfillPlaceholders) {
          this.$parent.find('[placeholder]').placeholder({
            customClass: 'mz-placeholder'
          });
        }
      },
      ...
      displayMessage: function (msg) {
        ...
        this.$parent
          .find('[data-mz-role="popover-message"]')
          .html('<span class="mz-validationmessage">' + msg + '</span>');
      },
    ...
    $.extend(LoginPopover.prototype, {
      ...
      login: function () {
        ...
        api
          .action('customer', 'loginStorefront', {
      ...
      anonymousorder: function () {
        ...
        }, _.bind(this.retrieveErrorLabel, this));
      },
    ...
    $.extend(SignupPopover.prototype, LoginPopover.prototype, {
      ...
      validate: function (payload) {
        if (!payload.account.emailAddress) {
          return this.displayMessage(Hypr.getLabel('emailMissing'));
          // false;
        }

        if (!payload.password) {
          return this.displayMessage(Hypr.getLabel('passwordMissing'));
          // false;
        }

        if (payload.password !== this.$parent.find('[data-mz-signup-confirmpassword]').val()) {
          return this.displayMessage(Hypr.getLabel('passwordsDoNotMatch'));
          // false;
        }
